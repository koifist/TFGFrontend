...
export class AdminComponent implements OnInit {
  ...
  public upgradeBroker(): void {
    if (this.brokerForm.valid) {
      this.userService.upgradeBroker(this.brokerForm.get('broker').value)
        .pipe(first())
        .subscribe(
          res => {
          ...
          err => {
            if (err.status === 400) {
              this.toastr.error('El usuario que ha introducido no existe');
            }
            this.toastr.error('Ha ocurrido un error en el servidor.');
          });
    } else {
      this.toastr.error('Introduzca un mensaje valido');
    }
  }
  ngOnInit() {
